...
QORIQ_TRANSAPI_VERSION = v0.2
QORIQ_TRANSAPI_SITE = https://github.com/openil/transAPI.git
QORIQ_TRANSAPI_SITE_METHOD = git
QORIQ_TRANSAPI_LICENSE = MIT/GPL2.0
QORIQ_TRANSAPI_LICENSE_FILES = LICENSE
# if use libtool to install libraries, flowing command is necessary
QORIQ_TRANSAPI_AUTORECONF = YES
QORIQ_TRANSAPI_DEPENDENCIES = libxml2 pyang libnetconf netopeer tsntool yang-model
QORIQ_TRANSAPI_CONF_ENV += PKG_CONFIG_PATH="$(STAGING_DIR)/usr/lib/pkgconfig"
QORIQ_TRANSAPI_CONF_ENV += PYTHON_CONFIG="$(STAGING_DIR)/usr/bin/python-config"
QORIQ_TRANSAPI_CONF_ENV += ac_cv_path_NETOPEER_MANAGER="$(HOST_DIR)/usr/bin/netopeer-manager.host"

LXML_MODULE_DIR = lxml-$(PYTHON_LXML_VERSION)-py2.7-linux-x86_64.egg
QORIQ_TRANSAPI_CONF_ENV += PYTHONPATH=$(HOST_DIR)/usr/lib/python$(PYTHON_VERSION_MAJOR)/site-packages/$(LXML_MODULE_DIR)
...
define QORIQ_TRANSAPI_COPY_IF_MODELS
...
define QORIQ_TRANSAPI_COPY_BRIDGE_MODELS
	$(INSTALL) -D -m 0755 $(BUILD_DIR)/yang-model-$(YANG_MODEL_VERSION)/standard/ietf/RFC/ietf-interfaces@2014-05-08.yang \
...
# create configure
define QORIQ_TRANSAPI_CREATE_CONFIGURE
        cd $(@D); \
	$(TARGET_MAKE_ENV) $(QORIQ_TRANSAPI_CONF_ENV) $(HOST_DIR)/usr/bin/lnctool \
	...
	$(TARGET_MAKE_ENV) $(QORIQ_TRANSAPI_CONF_ENV) $(HOST_DIR)/usr/bin/lnctool \
        ...
        cd $(@D); \
        $(TARGET_MAKE_ENV) $(QORIQ_TRANSAPI_CONF_ENV) $(HOST_DIR)/usr/bin/autoreconf --force --install
endef

# create Makefile
define QORIQ_TRANSAPI_CONFIGURE_CMDS
        cd $(@D); \
        $(TARGET_CONFIGURE_ARGS) $(TARGET_CONFIGURE_OPTS) $(QORIQ_TRANSAPI_CONF_ENV) \
...
define QORIQ_TRANSAPI_LNCTOOL_CREATE_FILES
        cd $(@D)/$(IF_SRC_NAME); \
        $(TARGET_MAKE_ENV) $(QORIQ_TRANSAPI_CONF_ENV) $(HOST_DIR)/usr/bin/lnctool \
...
QORIQ_TRANSAPI_PRE_CONFIGURE_HOOKS += QORIQ_TRANSAPI_COPY_IF_MODELS
QORIQ_TRANSAPI_PRE_CONFIGURE_HOOKS += QORIQ_TRANSAPI_COPY_BRIDGE_MODELS
QORIQ_TRANSAPI_PRE_CONFIGURE_HOOKS += QORIQ_TRANSAPI_COPY_PLATFORM
QORIQ_TRANSAPI_PRE_CONFIGURE_HOOKS += QORIQ_TRANSAPI_CREATE_CONFIGURE
QORIQ_TRANSAPI_POST_BUILD_HOOKS += QORIQ_TRANSAPI_LNCTOOL_CREATE_FILES
